...
def parseRestaurantList(url):
    ...
    restaurantList = []
    cnt = 0
    flist = open("restaurant_list.json",'w')
    for i in xrange(0,page_no/2):
        nurl = m.group(1)+'-'+m.group(2)+'-'+'oa'+(str)(i*30)+'-'+m.group(3)
        print nurl
        if nurl in visited_url.keys(): 
            continue
    ...
    for rest in restaurantList:
        ...
        if idx < 2580:
            continue
        parsedItem = parseRestaurant(rest)
...
def parseAttractionList(url):
    ...
    print "attraction list length:",len(attractionList)
    res = []
...
if __name__ == "__main__":
    ...
    root_url="http://www.tripadvisor.com/Tourism-g35805-Chicago_Illinois-Vacations.html"
    visited_url={}
